...
%     pause()
    ...
    for i=1:length(Pxy)
        ...
        for j=1:length(Pxy)
%             Pxy_new(j) = Pxy_new(j)+Pxy(i)*velocity(Xxy(:,j),Xxy(:,i),U(n,1),U(n,2),dt);
            Pxy_new(j) = Pxy_new(j)+Pxy(i)*odometry([Xxy(:,j);Z(3)],[Xxy(:,i);phi_old],mu1,[Xxy(:,i);phi_old]); 
